...
AlphaX.addCommand({pattern: 'link$', fromMe: WType, desc: DESC }, (async (message, match) => {
    ...
    try {
        ...
        exec('mv ' + location + ' /root/WhatsAsenaDuplicated/media/img_to_link.png')
        
            uploadByBuffer(fs.readFileSync('telegra-ph-upload.png'))
              .then(async (result) => {
             await message.client.sendMessage(
                    message.jid,
                    fs.readFileSync('/root/WhatsAsenaDuplicated/media/img_to_link.png'),
                    MessageType.image,
                    { caption: '*✅ Fɪʟᴇ sᴜᴄᴄᴇssғᴜʟʏ ᴜᴘʟᴏᴀᴅᴇᴅ ᴛᴏ* _telegra.ph_\n\n' + `*🔺ʟɪɴᴋ ➙*  _${result.link}_ \n*🔩️ ᴘᴀᴛʜ ➙* _${result.path}_ \n`, thumbnail: fs.readFileSync('/root/WhatsAsenaDuplicated/media/img_to_link.png' )}
                 );
            });
